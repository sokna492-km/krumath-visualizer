...
## This app (krumath-visualizer)

| Setting | Value |
| --- | --- |
| **App slug / path** | `/math-visualizer` |
| **Cloudflare Worker** | `math-visualizer` |
| **Gate style** | **Soft gate** — app usable unsigned; **Export PNG** requires signed-in (non-anonymous) user |
| **Sign-in return** | `/sign-in?returnUrl=/math-visualizer` |
| **Vite / Nitro base** | `/math-visualizer/` |

Do **not** edit the KruMath monorepo from this repo. Home-card on `/home` is maintainer Phase C only.
